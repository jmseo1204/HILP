"""
HILP with Dual Goal Representations (arXiv:2510.06714).

Key differences from hilp.py:
  - Value function: V(s,g) = psi(s)^T phi(g)  (inner product, not L2 distance)
  - psi and phi are separate encoders (state vs goal)
  - Skill reward: (psi(s') - psi(s)) · z  (same structure, different encoder)
  - Skill direction at eval: z = phi(g) / ||phi(g)||  (dual goal representation)
"""
...
from src.special_networks import GoalConditionedValue, GoalConditionedCritic, DualGoalPhiValue, HILPDualNetwork
...
def loss_fn(network_params, agent, batch):
    ...
    # Phase 1: GCVF with V(s,g) = psi(s)^T phi(g)
    value_loss, value_info = compute_value_loss(agent, batch, network_params)
    for k, v in value_info.items():
        info[f'value/{k}'] = v

    # Phase 2: Skill policy using psi(s) as intrinsic representation
    batch_size = batch['observations'].shape[0]
    rng, skill_rng = jax.random.split(rng)

    # psi(s) and psi(s'): state representations for skill reward
    batch['phis'] = agent.network(batch['observations'], method='phi')          # psi(s)
    batch['next_phis'] = agent.network(batch['next_observations'], method='phi') # psi(s')
    ...
    # Intrinsic reward: direction in psi-space (same structure as HILP)
...
class HILPDualAgent(flax.struct.PyTreeNode):
    rng: PRNGKey
    network: TrainState
    ...
    @jax.jit
    def get_phi(agent, observations: np.ndarray) -> jnp.ndarray:
        """psi(s): state representation, used for planning and skill direction delta."""
        return agent.network(observations, method='phi')

    @jax.jit
    def get_phi_goal(agent, goals: np.ndarray) -> jnp.ndarray:
        """phi(g): dual goal representation.
        At eval time, skill = phi(g) / ||phi(g)||.
        """
        return agent.network(goals, method='phi_goal')
...
def create_learner(
    ...
    value_def = DualGoalPhiValue(
        hidden_dims=value_hidden_dims, use_layer_norm=use_layer_norm,
        ensemble=True, skill_dim=skill_dim, encoder=encoder_module,
    )
    ...
    config = flax.core.FrozenDict(dict(
        ...
        skill_dim=skill_dim, skill_expectile=skill_expectile,
        skill_temperature=skill_temperature, skill_discount=skill_discount,
    ))

    return HILPDualAgent(rng, network=network, config=config)
